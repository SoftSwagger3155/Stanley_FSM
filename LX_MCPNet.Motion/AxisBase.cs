...
namespace LX_MCPNet.Motion
{
    ...
    public abstract class AxisBase : DispatcherObject, INotifyPropertyChanged, IDisposable
    {
        ...
        public bool IsProhibitActivated
        {
            get
            {
                return this._isProhibitActivated;
            }
        }
        ...
        {
            return step * this.mmPerStep;
        }

        public double get_step(double mm)
        {
            return mm * this.StepPermm;
        }
        ...
        {
            AxisBase.MotionStatus result = AxisBase.MotionStatus.MT_DONE;
            if (this.MtrTable.pCriticalEvent != null)
            {
                if (this.MtrTable.pCriticalEvent())
                {
                    this.SetInhibit(true);
                    result = AxisBase.MotionStatus.MT_CRITICAL;
            ...
            if (this.MtrTable.pFwdSensing != null)
            {
                if (this.MtrTable.pFwdSensing())
                {
                    this.SetInhibit(true);
                    this.sd_stop(0.0);
                    result = AxisBase.MotionStatus.MT_LIMIT;
                }
                else
                {
                    this.SetInhibit(false);
        ...
        public bool IsProhibitToMove(double distance)
        {
            ...
            this._isProhibitActivated = false;
            ...
            else
            {
                if (this.MtrTable.pIsInhibitToMove != null)
                {
                    ...
                    {
                        this._isProhibitActivated = true;
        ...
        public void StartInhibitThread()
        {
            ...
            {
                string strErr = "";
                this._endInhibitThread = false;
                Thread.MemoryBarrier();
                this._inhibitThread = delegate ()
                {
                    while (!this._endInhibitThread)
                    {
        ...
        protected AxisBase.MotionStatus SimulateMotionStatusCheck()
        {
            AxisBase.MotionStatus result = AxisBase.MotionStatus.MT_DONE;
            ...
            if (this.isStopReq)
            {
                result = AxisBase.MotionStatus.MT_DONE;
            }
            else if (timeSpan2.TotalMilliseconds < timeSpan.TotalMilliseconds)
            {
                ...
                result = AxisBase.MotionStatus.MT_MOVING;
            }
        ...
        public virtual AxisBase.MotionStatus check_motion_status()
        {
            return AxisBase.MotionStatus.MT_ALARM;
        }
        
        public void StopInhibitThread()
        {
            this._endInhibitThread = true;
            Thread.MemoryBarrier();
            Thread.Sleep(50);
        ...
        private MtrTable mtrTable;
        private MtrSpeed mtrSpeed;
        private MtrConfig mtrConfig;
        private MtrMisc mtrMisc = null;
        
        public static List<AxisBase> MotorsList = new List<AxisBase>();
        ...
        protected AxisBase.MotionStatus customHomingFlag = AxisBase.MotionStatus.MT_DONE;
        ...
        protected bool _endInhibitThread = false;
        protected bool _isProhibitActivated = false;
        ...
        private double tempPositionOffset = 0.0;
        private double statusReadingTime = 0.0;
        private List<PosMap> tPosMap = new List<PosMap>();
        private int mapSize = 300;
        private AxisBase.LastMoveDirection lastDirection = AxisBase.LastMoveDirection.Unknown;
        private MtrMotionProfile longDistanceProf = null;
        private double desiredPos = 0.0;
        private IMotionProfile lastMotionProfile = null;
        private static double disableInterLockAllowDistance = 10.0;
